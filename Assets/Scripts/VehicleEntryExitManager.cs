using UnityEngine;
using DevionGames.InventorySystem; // Devion 인벤토리 사용
...
public class VehicleEntryExitManager : MonoBehaviour
{
        ...
        if (Input.GetKeyDown(KeyCode.E))
        {
            ...
            else
            {
                ...
                if (player != null /* && IsPlayerNear(player) */)
                {
                    ...
                    if (!HasCarKey())
                    {
                        Debug.Log("[Vehicle] 인벤토리에 차 키가 없어서 탑승할 수 없습니다.");
                        return;
                    }
    ...
    {
        // 인벤토리 창에서 이름으로 아이템 찾기
        Item keyItem = ItemContainer.GetItem(inventoryWindowName, keyItemName);

        if (keyItem == null)
        {
            Debug.Log($"[Vehicle] 키 아이템(\"{keyItemName}\")을(를) \"{inventoryWindowName}\" 창에서 찾지 못했습니다.");
            return false;
        }
        ...
        if (!hasItem)
        {
            Debug.Log($"[Vehicle] \"{inventoryWindowName}\" 창에 \"{keyItemName}\" 아이템이 1개 이상 없습니다.");
        }
